...
serverAddress = ""
hedgehogId=0
serverPort = '733' + str(hedgehogId)
def test():
    #serverAddress = "hedegehog" + e1.get()
    #address = "hedegehog" + e1.get() + ".local"
    serverAddress = e1.get()
    print(serverAddress)
    socket.connect('tcp://%s:%s' % (serverAddress, serverPort))
    print('tcp://%s:%s' % (serverAddress, serverPort))
    master.destroy()


inIDE = False
lastAxisEventValue = {'left_trigger': 0.0, 'right_trigger': 0.0, 'l_thumb_x': 0.0, 'l_thumb_y': 0.0, 'r_thumb_x': 0.0,
                      'r_thumb_y': 0.0, '13': 0, '14': 0, '15': 0, '16': 0, '5': 0, '6': 0}
axisScaling = {'left_trigger': 1, 'right_trigger': 1, 'l_thumb_x': 0.5, 'l_thumb_y': 0.5, 'r_thumb_x': 0.5,
               'r_thumb_y': 0.5}
updateRate = int(250)
valueResetKey = int(6)
...
if not inIDE:
    master = Tk()
    Label(master, text='1. Hedgehog IP Adresse').grid(row=0)
    e1 = Entry(master)
    e1.grid(row=0, column=1)
    button = Button(master, text="ok", command=test).grid(row=5)
    hedgehogId = 0
    mainloop()
...
def determine_optimal_sample_rate(joystick=None):
    ...
    #  performance
    if joystick is None:
        joystick = XInputJoystick.enumerate_devices()[hedgehogId]
...
def sample_first_joystick():
    ...
    def on_button(button, pressed):
        ...
        if (button == valueResetKey and pressed == int(1)):
            for axis in lastAxisEventValue:
                lastAxisEventValue[axis] = 0.0
            lastAxisEventValue[str(valueResetKey)] = 1
        elif (button == valueResetKey and pressed == int(0)):
            for axis in lastAxisEventValue:
                print('%s: %d' % (axis, lastAxisEventValue[axis]))
            lastAxisEventValue[str(valueResetKey)] = 0
        elif (button == int(5) and pressed == int(1)):
            print('Killing car and application')
            lastAxisEventValue['5'] = 1
    ...
    def on_axis(axis, value):
        left_speed = 0
        right_speed = 0
        manipulatedValue = round(value / axisScaling[axis], 3)
        if (float(manipulatedValue) > 0.9):
            manipulatedValue = 1.0
        elif (float(manipulatedValue) < -0.9):
            manipulatedValue = -1.0
        elif (abs(float(manipulatedValue)) < 0.333 and (axis != 'left_trigger' and axis != 'right_trigger')):
            manipulatedValue = 0.0
        elif (abs(float(manipulatedValue)) < 0.1):
            manipulatedValue = 0.0
        if (lastAxisEventValue[axis] != manipulatedValue):
            lastAxisEventValue[axis] = manipulatedValue
            print('axis', axis, lastAxisEventValue[axis])
    ...
    while True:
        now = datetime.now()
        deltaTimeEvent = now - lastEvent
        deltaTimeEvent = int(deltaTimeEvent.total_seconds() * 1000)
        deltaTimeSend = now - lastSend
        deltaTimeSend = int(deltaTimeSend.total_seconds() * 1000)
        if deltaTimeEvent > 10:
            j.dispatch_events()
            lastEvent = now
        if deltaTimeSend > updateRate:
            # sends current state to specified device
            # socket.send_json()
            #print(lastAxisEventValue)
            socket.send_json(json.dumps(lastAxisEventValue))
            lastSend = now
            if lastAxisEventValue['5'] == int(1):
